...
`digest` follows the same Gemma 3 runtime path as `smoke_test_g3.py`: Gemma 3
4B IT, multimodal `ChatSampler`, `print_stream=True`, `cache_length=10240`, and
the same JAX memory environment settings.
...
digestion.json       run state, active plan, chunk statuses, rolling memory
...
incomplete chunk while preserving completed summaries and rolling memory.

Rolling memory is append-first. Each chunk returns a `MEMORY_DELTA` containing
only durable new facts; the runner appends that delta to the existing memory. If
the memory approaches its internal budget, the runner prints and logs a separate
compaction step, streams the compaction output into the Markdown file, and then
continues.
